...
function count_job($src_param, $post_order_by, $query_per_page, $cat){
   ...
   $sql    = "SELECT * FROM tbl_store AS job INNER JOIN tbl_store_category AS dept ON job.category = dept.category_id
              WHERE $src_param $cat
...
function get_detail($post_career_id){
   ...
   $sql    = "SELECT * FROM tbl_store WHERE `career_id` = '$post_career_id'";
   $query  = mysql_query($sql, $conn);
   $result = mysql_fetch_array($query);
