...
$full_order       = count_job($search_query, $query_per_page);
$total_query      = $full_order['total_query'];
$total_page       = ceil($full_order['total_query'] / $query_per_page);

// CALL FUNCTION
$listing_order    = get_job($search_query, $sort_by, $first_record, $query_per_page);
...
// STORED VALUE
echo "<input type=\"hidden\" name=\"url\" id=\"url\" class=\"hidden\" value=\"http://".$_SERVER['HTTP_HOST'].get_dirname($_SERVER['PHP_SELF'])."/filter-substrat-view\">\n";
