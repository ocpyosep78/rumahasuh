...
if($_POST['btn-index-category'] == "Save Changes"){
   ...
   //}else if(!empty($category_id)){
	  ...
	  $_SESSION['msg']   = "Item(s) has been successfully added.";
	   ...
	   $new_id          = mysql_query("SELECT MAX(category_id) AS new_id FROM tbl_tag",$conn);
	   ...
	   $delete_relation = mysql_query("DELETE FROM tbl_tag_relation WHERE `category_child` = '$category_id'",$conn);
	   $delete_root     = mysql_query("DELETE FROM tbl_tag WHERE `category_id` = '$category_id'",$conn);
   ...
   if($validate_delete['total_product'] > 0){
      ...
      $get_order = mysql_query("SELECT * from tbl_tag WHERE category_id = '$category_id'",$conn);
      ...
      mysql_query("UPDATE tbl_tag SET category_order = category_order-1 WHERE category_order > '$category_order'",$conn);
   
      //$category_parent_redirect = $_GET["parent_category"];
      mysql_query("DELETE FROM tbl_tag WHERE category_id = '$category_id'",$conn);
   
      $category_relation = mysql_query("SELECT * from tbl_tag_relation WHERE category_parent = '$category_id'",$conn);
      ...
      if (mysql_num_rows($category_relation)!=null){
	     ...
	     for ($counter=1;$counter<=mysql_num_rows($category_relation);$counter++){
		    ...
		    mysql_query("DELETE FROM tbl_tag_relation WHERE category_parent = '$category_child'",$conn);
