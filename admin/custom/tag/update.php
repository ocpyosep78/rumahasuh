...
function add_category_relation($one, $two, $three, $four){
   ...
   $sql    = "INSERT INTO tbl_tag_relation 
	  ...
	  */
	  ...
	  if (mysql_num_rows($category_relation)!=null){
	     ...
	     for ($counter=1;$counter<=mysql_num_rows($category_relation);$counter++){
			...
			mysql_query("DELETE FROM tbl_tag_relation WHERE category_parent = '$category_child' OR category_child = '$category_child'",$conn);
...
function update_order($post_order, $post_category_id){
   ...
   $sql   = "UPDATE tbl_tag SET `category_order` = '$post_order' WHERE `category_id` = '$post_category_id'";
