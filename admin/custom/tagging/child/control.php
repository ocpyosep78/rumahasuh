...
if($_POST['btn_child_tagging'] == "Save Changes"){
   ...
   if(empty($category_id)){
	  ...
	  mysql_query(" INSERT INTO tbl_tags
      ...
      $get_id = mysql_query("SELECT * from tbl_tags WHERE category_name = '$category_name' ORDER BY category_id DESC",$conn);
	  ...
	  mysql_query("INSERT INTO tbl_tags_relation(category_child,category_parent,relation_level) VALUES('$category_id','$category_parent','1')",$conn);
   ...
   //}else if(!empty($category_id)){
      mysql_query("UPDATE tbl_tags SET category_level = '$category_level' WHERE category_id = '$category_id'",$conn);
	  ...
	  $_SESSION['msg']   = "Item(s) has been successfully added.";
	   ...
	   $new_id          = mysql_query("SELECT MAX(category_id) AS new_id FROM tbl_tags",$conn);
	   
	   $new_id = (int) $new_id['new_id'] + 1;
	   $update_product  = mysql_query("UPDATE tbl_product SET `product_category` = '$new_id' WHERE `product_category` = '$category_id' MAX(category_id) AS new_id FROM tbl_tag",$conn);
	   
	   $delete_relation = mysql_query("DELETE FROM tbl_tags_relation WHERE `category_child` = '$category_id'",$conn);
	   $delete_root     = mysql_query("DELETE FROM tbl_tags WHERE `category_id` = '$category_id'",$conn);
...
}else if($_POST['btn_child_tagging'] == "Delete"){
   // DEFINED VARIABLE
   $category_id   = $_POST['cat_id'];
   ...
   if($validate_delete['total_product'] > 0){
      ...
      if (mysql_num_rows($category_relation)!=null){
	     ...
	     for ($counter=1;$counter<=mysql_num_rows($category_relation);$counter++){
		    ...
		    mysql_query("DELETE FROM tbl_tags_relation WHERE category_parent = '$category_child'",$conn);
...
}else if($_POST['btn_child_tagging'] == "GO"){
...
function listCategory($level,$parent,$current_category){
   ...
   $get_data = mysql_query("SELECT * from tbl_tags AS cat INNER JOIN tbl_tag_relation AS cat_rel ON cat.category_id = cat_rel.category_child
...
function showCategory($level,$parent,$current_category, $one, $two, $three, $four,$tot_query, $row){
   ...
   $get_data = mysql_query("SELECT 
							...
							FROM tbl_tags AS cat LEFT JOIN tbl_product AS prod ON cat.category_id = prod.product_category
							                          INNER JOIN tbl_tags_relation AS rel ON cat.category_id = rel.category_child
...
if(isset($_POST['btn_child_tagging'])){
