...
function count_category($search, $sort_by, $qpp){
   ...
   $sql   = "SELECT * FROM tbl_tags AS cat INNER JOIN tbl_tags_relation AS rel ON cat.category_id = rel.category_child
...
function get_total_product($category_id){
   ...
   $sql    = "SELECT IFNULL(total_product_main,0)+IFNULL(total_product_child,0) AS total_product
			 ...
			 LEFT JOIN (SELECT * from tbl_tags_relation WHERE relation_level = '1') AS relation
