	  ...
	  */
	  ...
	  if (mysql_num_rows($category_relation)!=null){
	     ...
	     for ($counter=1;$counter<=mysql_num_rows($category_relation);$counter++){
			...
			mysql_query("DELETE FROM tbl_category_relation WHERE category_parent = '$category_child' OR category_child = '$category_child'",$conn);
...
function update_order($post_order, $post_category_id){
   ...
   $sql   = "UPDATE tbl_category SET `category_order` = '$post_order' WHERE `category_id` = '$post_category_id'";
